...
// Theme Management
function initTheme() {
    // Suppress transitions while applying the saved theme so there's no flash on load
...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        const sourceChips = sources.map(s =>
            s.url
                ? `<a class="source-chip" href="${s.url}" target="_blank" rel="noopener noreferrer">${escapeHtml(s.label)}<span class="source-chip-icon">↗</span></a>`
                : `<span class="source-chip">${escapeHtml(s.label)}</span>`
        ).join('');
        html += `
